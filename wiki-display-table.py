...
def show_dataframe():
    ...
    # Convert the DataFrame to Pandas, keep only top 10 entries
    title_counts_pd = title_counts.toPandas().head(10)

    # Convert the DataFrame to HTML table
    html = title_counts_pd.to_html(index=False, classes='table table-striped')
    ...
    return render_template_string("""
        ...
        <body>
            <div class="container">
                <h1 class="mt-5">Wiki Title Changes Count</h1>
                {{ html|safe }}
            </div>
        </body>
        </html>""", html=html)
